...
def scan_directory(model_path: str | Path) -> list[dict[str, Any]]:
    ...
    if not path.exists():
        return [{"severity": "error", "check": "path_exists", "detail": f"Path does not exist: {path}"}]
...
def _check_weight_anomalies(path: Path, findings: list[dict[str, Any]]) -> None:
    ...
    if len(shards) == 1 and shards[0].stat().st_size < 100_000_000:
        findings.append({
            ...
            "evidence": {"file": str(shards[0]), "size_mb": round(shards[0].stat().st_size / 1e6, 1)},
        })
...
def detect_hook_registration(source: str = "auto") -> dict[str, Any]:
    result: dict[str, Any] = {
            ...
            if hasattr(torch_mod, "nn"):
                ...
                if hasattr(nn_mod, "Module"):
                    ...
                    if orig and hasattr(orig, "__wrapped__"):
                        ...
                        result["risk"] = "high"
                        result["detail"] = "torch.nn.Module.register_forward_hook appears to be wrapped — active hook monitoring"
    except Exception:
...
def generate_report(path: str | Path) -> str:
    ...
    for f in findings:
        icon = {"critical": "[!!]", "high": "[!]", "warning": "[?]", "info": "[i]", "error": "[E]"}.get(
            f.get("severity", "info"), "[?]"
        )
